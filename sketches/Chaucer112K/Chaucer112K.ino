...
void setup();
void loop();
const char knightsTale [] __attribute__((section(".fini7"))) = 
"                     THE KNIGHT'S TALE <1>\n"
"\n"
"\n"
...
const char knightsTale2 [] __attribute__((section(".fini7"))) = 
...
;
const char knightsTale3 [] __attribute__((section(".fini7"))) = 
...
void setup() {
  ...
  address = pgm_get_far_address(knightsTale);
  while (c = pgm_read_byte_far(address)) {
    Serial.write(c);
    
    if ((address % 1024) == 0)
      digitalWrite(LED, (address/1024)&1);
    
    address++;
  } 

  address = pgm_get_far_address(knightsTale2);
  while (c = pgm_read_byte_far(address)) {
    Serial.write(c);
  ...
  address = pgm_get_far_address(knightsTale3);
  while (c = pgm_read_byte_far(address)) {
    Serial.write(c);
